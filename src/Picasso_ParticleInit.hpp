...
void filterEmpties(const ExecutionSpace &exec_space, const int local_num_create,
  ...
  Kokkos::parallel_scan(
      ...
      KOKKOS_LAMBDA(const int i, int &count, const bool final_pass) {
        if (particle_created(i - previous_num_particles)) {
          if (final_pass) {
            particles.setTuple(indices(count), particles.getTuple(i));
          }
          ++count;
        }
      });
...
void initializeParticles(InitUniform, const ExecutionSpace &exec_space,
  ...
  int particles_per_cell =
      particles_per_cell_dim * particles_per_cell_dim * particles_per_cell_dim;
  int num_particles = particles_per_cell * owned_cells.size();
  int previous_num_particles = particles.size();
  particles.resize(previous_num_particles + num_particles);
          ...
          for (int jp = 0; jp < particles_per_cell_dim; ++jp)
            for (int kp = 0; kp < particles_per_cell_dim; ++kp) {
              ...
              int pid =
                  ...
                  particles_per_cell_dim * (jp + particles_per_cell_dim * kp);
...
void initializeParticlesSurface(InitRandom, const ExecutionSpace &,
  ...
  // Create a random number generator.
  uint64_t pool_seed = global_grid.blockId() + seed;
  using rnd_type = Kokkos::Random_XorShift64_Pool<ExecutionSpace>;
  rnd_type pool;
  pool.init(pool_seed, num_facets);

  // Get the surface mesh vertices
  auto &surface_verts = surface.vertices;
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int f) {
        ...
        auto a = surface_verts.getParticleView(surface.facet_ids(f, 0));
        auto b = surface_verts.getParticleView(surface.facet_ids(f, 1));
        auto c = surface_verts.getParticleView(surface.facet_ids(f, 2));

        // Get vertex positions
        auto a_x = Picasso::get(a, Picasso::Field::PhysicalPosition<3>());
        auto b_x = Picasso::get(b, Picasso::Field::PhysicalPosition<3>());
        auto c_x = Picasso::get(c, Picasso::Field::PhysicalPosition<3>());

        // Get vertex normals
        auto a_n = Picasso::get(a, Picasso::Field::VertexNormal<3>());
        auto b_n = Picasso::get(b, Picasso::Field::VertexNormal<3>());
        auto c_n = Picasso::get(c, Picasso::Field::VertexNormal<3>());

        // Vec3<double> a = {surface.facets(f, 0, 0), surface.facets(f, 0, 1),
        //                   surface.facets(f, 0, 2)};
        //
        // Vec3<double> b = {surface.facets(f, 1, 0), surface.facets(f, 1, 1),
        //                   surface.facets(f, 1, 2)};
        //
        // Vec3<double> c = {surface.facets(f, 2, 0), surface.facets(f, 2, 1),
        //                   surface.facets(f, 2, 2)};
        //
        // Vec3<double> ab = b - a;
        // Vec3<double> ac = c - a;
        // Vec3<double> pan =
        Vec3<double> ab = b_x - a_x;
        Vec3<double> ac = c_x - a_x;
        ...
        for (int ip = 0; ip < particles_per_facet; ++ip) {
          ...
          //
          auto sum = Kokkos::rand<decltype(rand), double>::draw(rand, 0.0, 1.0);
          auto u = Kokkos::rand<decltype(rand), double>::draw(rand, 0.0, sum);
          auto v = sum - u;
          auto w = 1.0 - u - v;

          // Interpolate the position and normal vector to the point
          px = u * a_x + v * b_x + w * c_x;
          pan = u * a_n + v * b_n + w * c_n;

          // Normalize the normal vector
          auto pan_mag = Kokkos::hypot(pan(0), pan(1), pan(2));
          pan = pan / (pan_mag + 1e-14);

          // We need two random numbers such that r + s <= 1
          // auto sum = Kokkos::rand<decltype(rand), double>::draw(rand,
          // 0.0, 1.0); auto r = Kokkos::rand<decltype(rand),
          // double>::draw(rand, 0.0, sum); auto s = sum - r;
          //
          ...
          // Create a new particle with the given logical
          // coordinates.
          bool created = create_functor(px.data(), pan.data(), pa, particle);
          if (created) {
